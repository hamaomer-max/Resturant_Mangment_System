...
<table id="myTable" class="table table-bordered">
    <thead>
      <tr>
        ...
        <th class="bg-dark text-light rounded text-center">Table Number</th>
        <th class="bg-dark text-light rounded text-center">Set Reservation</th>
...
$(function() {
  let table = $('#myTable').DataTable({
    ...
    columns: [ 
      ...
      {data: 'table_num', name: 'table_num'},
    
      { 
      data: 'set_reservation', 
      name: 'set_reservation', 
      orderable: false, 
      searchable: false, 
      render: function(data, type, row){
      const id = row.id;
      const routeTemplate = '{{ route('admin.ressrevations.show', ':id') }}';
      const route = routeTemplate.replace(':id', id);
      return `<a href="${route}" class="btn btn-sm btn-info mb-2 w-100 text-light" style="width: 55px">Set</a>`;
  }
